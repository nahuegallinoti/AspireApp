...
{
    private static readonly PathResolver Paths = new(@"C:\fake\root");
    ...
    {
        SharedFileNames(GenerationPlan.Build(Specs.Entity(blazor: true, nav: true), Paths)).Should().Contain("NavMenu.razor");
        SharedFileNames(GenerationPlan.Build(Specs.Entity(blazor: true, nav: false), Paths)).Should().NotContain("NavMenu.razor");
        SharedFileNames(GenerationPlan.Build(Specs.Entity(blazor: false, nav: true), Paths)).Should().NotContain("NavMenu.razor");
    }
    ...
    public void ServerModeAddsPagingProjectReferenceMutator()
    {
        ...
        SharedFileNames(GenerationPlan.Build(Specs.Entity(mode: FilterMode.Client), Paths))
            .Should().NotContain("AspireApp.Application.Models.csproj");
        SharedFileNames(GenerationPlan.Build(Specs.Entity(mode: FilterMode.Server), Paths))
