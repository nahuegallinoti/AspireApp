...
namespace AspireApp.Tools.Generator.Tests;
...
public class TemplateRendererTests
{
    ...
    {
        var tokens = TemplateRenderer.BuildTokens(Specs.Entity(name: "Order", idType: "long"));
    ...
    public void BuildTokensMapsIdRouteConstraint(string idType, string expected) =>
        TemplateRenderer.BuildTokens(Specs.Entity(idType: idType))["ID_ROUTE_CONSTRAINT"].Should().Be(expected);

    [Fact]
    public void AuthorizeTokensToggleWithRequireAuth()
    {
        TemplateRenderer.BuildTokens(Specs.Entity(auth: true))["AUTHORIZE_ATTR"].Should().Contain("[Authorize]");
        TemplateRenderer.BuildTokens(Specs.Entity(auth: false))["AUTHORIZE_ATTR"].Should().BeEmpty();
    }

    [Fact]
    public void EventBusTokensToggle()
    {
        var on = TemplateRenderer.BuildTokens(Specs.Entity(eventBus: true));
        on["EVENT_BUS_CTOR_PARAM"].Should().Contain("IMessageBus");
        on["EVENT_BUS_BASE_ARG"].Should().Contain("messageBus");

        TemplateRenderer.BuildTokens(Specs.Entity(eventBus: false))["EVENT_BUS_CTOR_PARAM"].Should().BeEmpty();
    }

    [Fact]
    public void DaBodyIsEmptyForClientAndImplementedForServer()
    {
        TemplateRenderer.BuildTokens(Specs.Entity(mode: FilterMode.Client))["DA_BODY"].Should().Be(";\n");

        var server = TemplateRenderer.BuildTokens(Specs.Entity(
            props: [Specs.Prop("Name", "string", filter: true)],
            mode: FilterMode.Server));
